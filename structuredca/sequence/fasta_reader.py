...
class FastaReader:
    ...
    def count_sequences(cls, fasta_path: str) -> int:
        ...
        if not os.path.isfile(fasta_path):
            raise ValueError(f"ERROR in StructureDCA.FastaStream(): fasta_path='{fasta_path}' file does not exist.")
        if not any([fasta_path.endswith(f".{ext}") for ext in FastaStream.ACCEPTED_MSA_EXTENSIONS]):
            raise ValueError(f"ERROR in StructureDCA.FastaStream(): fasta_path='{fasta_path}' should have extension among {FastaStream.ACCEPTED_MSA_EXTENSIONS}.")
...
class FastaStream:
    ...
    # Constants ----------------------------------------------------------------
    HEADER_START_CHAR = Sequence.HEADER_START_CHAR
    ACCEPTED_MSA_EXTENSIONS = ["fasta", "a2m", "a3m", "fasta.gz", "a2m.gz", "a3m.gz", "fa", "fasta.txt"]
    ...
    def __init__(self, fasta_path: str):
        ...
        if not os.path.isfile(fasta_path):
            raise ValueError(f"ERROR in StructureDCA.FastaStream(): fasta_path='{fasta_path}' file does not exist.")
        if not any([fasta_path.endswith(f".{ext}") for ext in self.ACCEPTED_MSA_EXTENSIONS]):
            raise ValueError(f"ERROR in StructureDCA.FastaStream(): fasta_path='{fasta_path}' should have extension among {self.ACCEPTED_MSA_EXTENSIONS}.")
        ...
        assert self.current_line is not None, f"ERROR in StructureDCA.FastaStream(): no sequences found in file '{fasta_path}'."
        assert self._is_current_line_header(), f"ERROR in StructureDCA.FastaStream(): first line of file '{fasta_path}' sould be a fasta header (thus start with '{self.HEADER_START_CHAR}').\nline='{self.current_line}'"
